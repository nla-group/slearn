...
os.makedirs("results_partial_high", exist_ok=True)


cache_dir = os.path.expanduser("~/.cache/torch_extensions") # Clear PyTorch extensions cache
...
def analyze_scaling_laws(results_df):
    ...
    scaling_df.to_csv('figures_high/scaling_laws.csv', index=False)
    ...
    for model_name in results_df['model'].unique():
        ...
        for metric in ['test_loss', 'test_accuracy']:
            ...
            plt.legend()
            plt.savefig(f'figures_high/scaling_model_size_{model_name}_{metric}.png', dpi=300)
            plt.close()
            ...
            plt.legend()
            plt.savefig(f'figures_high/scaling_dataset_size_{model_name}_{metric}.png', dpi=300)
            plt.close()
...
def generate_visualizations(results_df):
    ...
    for metric, title, ylabel, filename, log_scale, plot_type in metrics:
        ...
        if log_scale:
            plt.yscale('log')
        plt.tight_layout()
        plt.savefig(f'figures_high/{filename}', dpi=300, bbox_inches='tight')
        plt.close()
    ...
    for metric in ['test_loss', 'test_accuracy']:
        ...
        plt.ylabel(metric)
        plt.tight_layout()
        plt.savefig(f'figures_high/{metric}_vs_dataset_size.png', dpi=300)
        plt.close()
        ...
        plt.ylabel(metric)
        plt.tight_layout()
        plt.savefig(f'figures_high/{metric}_vs_model_size.png', dpi=300)
        plt.close()
    ...
    if stats_results:
        ...
        print("\nStatistical Tests:")
        print(stats_df)
        stats_df.to_csv('figures_high/statistical_tests.csv', index=False)
    ...
    if available_metrics:
        ...
        print(summary)
        summary.to_csv('figures_high/summary_statistics.csv')
