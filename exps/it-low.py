...
def extended_statistical_analysis(results_df):
    ...
    os.makedirs("figures_low/extended_stats", exist_ok=True)
    ...
    if pairwise_results:
        ...
        pairwise_df.to_csv('figures_low/extended_stats/pairwise_mannwhitney_tests.csv', index=False)
        logging.info("Saved pairwise Mann-Whitney U tests to figures_low/extended_stats/pairwise_mannwhitney_tests.csv")
    else:
    ...
    if friedman_results:
        ...
        friedman_df.to_csv('figures_low/extended_stats/friedman_tests.csv', index=False)
        logging.info("Saved Friedman tests to figures_low/extended_stats/friedman_tests.csv")
    else:
    ...
    with open('figures_low/extended_stats/lmm_results.txt', 'w') as f:
        for result in lmm_results:
            f.write(f"Metric: {result['Metric']}\n{result['LMM_Result']}\n\n")
    logging.info("Saved LMM results to figures_low/extended_stats/lmm_results.txt")
    ...
    if not corr_df.empty:
        corr_df.to_csv('figures_low/extended_stats/correlation_analysis.csv', index=False)
        logging.info("Saved correlation analysis to figures_low/extended_stats/correlation_analysis.csv")
    else:
    ...
    if available_metrics:
        ...
        summary.to_csv('figures_low/extended_stats/summary_statistics_with_trimmed.csv')
        logging.info("Saved summary statistics to figures_low/extended_stats/summary_statistics_with_trimmed.csv")
    else:
