...
complexities = list(range(13, 21))  # High LZW complexity: 13–20
min_length = 1100
window_size = 100
...
os.makedirs("figures_high", exist_ok=True)
...
plt.title('Training Time by Model and Layers (High Complexity)')
plt.ylabel('Time (seconds)')
plt.yscale('log')
plt.legend(title='Layers')
plt.tight_layout()
plt.savefig('figures_high/time_violin.png', dpi=300, bbox_inches='tight')
plt.close()

plt.figure(figsize=(12, 6))
sns.violinplot(x='model', y='DL', hue='layers', data=results_df, split=True)
plt.title('Normalized Damerau-Levenshtein Distance by Model and Layers (High Complexity)')
plt.ylabel('DL Distance')
plt.legend(title='Layers')
plt.tight_layout()
plt.savefig('figures_high/dl_violin.png', dpi=300, bbox_inches='tight')
plt.close()

plt.figure(figsize=(12, 6))
sns.violinplot(x='model', y='JW', hue='layers', data=results_df, split=True)
plt.title('Normalized Jaro-Winkler Distance by Model and Layers (High Complexity)')
plt.ylabel('JW Distance')
plt.legend(title='Layers')
plt.tight_layout()
plt.savefig('figures_high/jw_violin.png', dpi=300, bbox_inches='tight')
plt.close()

plt.figure(figsize=(12, 6))
sns.violinplot(x='model', y='epochs', hue='layers', data=results_df, split=True)
plt.title('Epochs to Convergence by Model and Layers (High Complexity)')
plt.ylabel('Epochs')
plt.legend(title='Layers')
plt.tight_layout()
plt.savefig('figures_high/epochs_violin.png', dpi=300, bbox_inches='tight')
plt.close()

plt.figure(figsize=(12, 6))
sns.violinplot(x='model', y='memory', hue='layers', data=results_df, split=True)
plt.title('Memory Usage by Model and Layers (High Complexity)')
plt.ylabel('Memory (MB)')
plt.legend(title='Layers')
plt.tight_layout()
plt.savefig('figures_high/memory_violin.png', dpi=300, bbox_inches='tight')
plt.close()
...
for metric in ['DL', 'JW']:
    ...
    plt.title(f'Mean {metric} by Layers and Units (High Complexity)')
    plt.tight_layout()
    plt.savefig(f'figures_high/{metric.lower()}_heatmap.png', dpi=300, bbox_inches='tight')
    plt.close()
...
print("\nSummary Table (High Complexity):")
print(summary)
summary.to_csv('figures_high/summary_table.csv')